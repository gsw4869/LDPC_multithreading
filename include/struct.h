...
typedef struct
{
	...
	long num_Error_Frames_1; // 当前SNR下的误判帧数(将错误(信息位有错误比特)的帧判为正确(校验结果正确)的帧)
	long num_Error_Bits_1;	 // 当前SNR下的虚警帧数(将正确(信息位无错误比特)的帧判为错误(校验结果错误)的帧)

	float FER;		 // =num_Error_Frames/num_Frames
	float BER;		 // =num_Error_Bits/num_Frames
	float AverageIT; // =Total_Iteration/num_Frames
	float FER_1;	 // =num_False_Frames/num_Frames
	float BER_1;	 // =num_Alarm_Frames/num_Frames
